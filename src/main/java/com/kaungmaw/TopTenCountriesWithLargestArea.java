...
public class TopTenCountriesWithLargestArea {
    ...
    public TopTenCountriesWithLargestArea(Connection conn) {
        this.conn = conn;
    }

    public void displayTopTenCountriesWithLargestArea() {
        try {
            PreparedStatement ps = conn.prepareStatement("SELECT country_name, iso_name, area_sq_km, world_rank, 2020_population, 2023_population FROM population ORDER BY area_sq_km DESC LIMIT 10");
            ResultSet rs = getTopTenCountriesWithLargestAreaFromDatabase();

            System.out.printf("%-25s%-15s%-35s%-15s%-15s\n", "Country Name", "ISO Name", "Country Area (Square Kilometer)", "Growth Rate", "World Rank");
            while (rs.next()) {
                System.out.printf("%-25s%-15s%-35s%-15s%-15s\n", rs.getString(1), rs.getString(2), rs.getInt(3), calculateGrowthRate(rs.getInt(5), rs.getInt(6)) + "%", rs.getInt(4));
            }
